...
public class PhotoListManager {
    ...
    public void insertOnTopDao(PhotoItemCollectionDao newdao)
    {
        if(dao == null)
            dao = new PhotoItemCollectionDao();
        if(dao.getData() == null)
            dao.setData(new ArrayList<PhotoListDao>());
        dao.getData().addAll(0,newdao.getData());
        saveCache();
    }

    public void insertOnTailDao(PhotoItemCollectionDao newdao)
    {
        if(dao == null)
            dao = new PhotoItemCollectionDao();
        if(dao.getData() == null)
    ...
    private void saveCache(){
        ...
        SharedPreferences pref = mContext.getSharedPreferences("photos",Context.MODE_PRIVATE);
